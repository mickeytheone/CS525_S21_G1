...
testRecords (void)
{
	...
	Value * stv = stringToValue("i1");
	OP_TRUE(stv, value, valueEquals, "first attr");
	free(stv);
	freeVal(value);

	getAttr(r, schema, 1, &value);
	Value * stv2 = stringToValue("saaaa");
	OP_TRUE(stv2, value, valueEquals, "second attr");
	free(stv2->v.stringV);
	free(stv2);
	freeVal(value);

	getAttr(r, schema, 2, &value);
	Value * stv3 = stringToValue("i3");
	OP_TRUE(stv3, value, valueEquals, "third attr");
	free(stv3);
	freeVal(value);

	//modify attrs
	Value * stv4 = stringToValue("i4");
	setAttr(r, schema, 2, stv4);
	getAttr(r, schema, 2, &value);
	OP_TRUE(stv4, value, valueEquals, "third attr after setting");
	free(stv4);
	freeVal(value);
        freeSchema(schema); //Added this
	freeRecord(r);
	TEST_DONE();
}
...
testCreateTableAndInsert (void)
{
	...
	for(i = 0; i < 1000; i++)
	{
		r = fromTestRecord(schema,inserts[0]); //Added - Dummy record created using test schema
		int pos = rand() % numInserts;
		RID rid = rids[pos];
		TEST_CHECK(getRecord(table, rid, r));
		temp = fromTestRecord(schema,inserts[pos]); //Added
		ASSERT_EQUALS_RECORDS(temp, r, schema, "compare records");
		freeRecord(r); //Added
		freeRecord(temp); //Added
	} 
...
testInsertManyRecords(void)
{
	...
	int randomRec = 3333;
       // int numInserts = 10, i; //For Test only
       // int randomRec = 3; //For test only
	Record *r;
	RID *rids;
	Schema *schema;
	...
	for(i = 0; i < numInserts; i++)
	{
		...
		char *mSer;
		char *rSer;
		getAttr(r, schema, 0, &lVal);
		getAttr(r, schema, 1, &mVal);
		getAttr(r, schema, 2, &rVal);
		lSer = serializeValue(lVal); 
		rSer = serializeValue(rVal);
		mSer = serializeValue(mVal);
		printf("%s\n",lSer);
		printf("%s\n",mSer);
		printf("%s\n",rSer);
		}
